...
function tabPos(){
  var sc=window.sched; if(!sc||!sc.on)return null;
  if(typeof window.curMpt!=='function')return null;
  var mpt=curMpt(); if(!mpt)return null;
  var t=sc.tickRef+(performance.now()-sc.msRef)/mpt;
  if(t<0)return null;                                  // Count-In
  var BARt=window.BAR||1920;
  var bar=Math.floor(t/BARt), frac=(t-bar*BARt)/BARt;
  return tabMode==='scroll' ? 4+frac : (bar%TABBARS)+frac;
}
(function(){
  function loop(){
    var ph=el('tabPlay');
    if(ph){
      var p=document.hidden?null:tabPos();
